...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!file || !(file instanceof File)) return fail("VALIDATION_ERROR", "الملف مطلوب", 422)

  const maxSize = 20 * 1024 * 1024
  if (file.size > maxSize) return fail("FILE_TOO_LARGE", "حجم الملف يتجاوز 20 ميجابايت", 422)
  ...
  const doc = await db.storedDocument.create({
    data: {
      caseId: id, originalName: file.name, storedName,
      mimeType: file.type || "application/octet-stream",
      fileSize: file.size, uploadedBy, sourceType,
      ocrStatus, ocrText: extraction.text ?? null,
