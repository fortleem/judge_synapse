...
// Accepts: file + optional metadata (uploadedBy, sourceType)
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const uploadedBy = (formData.get("uploadedBy") as string) || "rapporteur"
  ...
  try {
    ...
    return fail("UPLOAD_FAILED", `فشل حفظ الملف: ${err instanceof Error ? err.message : "خطأ غير معروف"}`, 500)
  }

  // Extract text (for OCR status)
  const extraction = extractTextFromFile(buffer, file.type, file.name)
  const ocrStatus = extraction.needsOcr ? "pending" : "completed"
  const ocrText = extraction.text
  ...
  const doc = await db.storedDocument.create({
    data: {
      ...
      ocrConfidence: ocrText ? 1.0 : null,
