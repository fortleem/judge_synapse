...
export async function POST(req: NextRequest) {
  ...
  const full = await db.case.findUnique({ where: { id: row.id }, include: { facts: true, evidence: true, timeline: true, issues: true, authorities: true, judgeFields: true, aiAnalyses: true, indicators: true, conflicts: true, adversaryReviews: true, notes: true, auditLogs: true, citationVerifications: true, deadlines: true, documents: true, partyMembers: true } })
